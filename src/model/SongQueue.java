...
/**
 * Represents a queue of songs chosen by the user. Provides methods for
 * validating, adding, and removing a given song.
 * 
...
public class SongQueue implements Serializable {
	 ...
	 *            to be added
	 * @param minsRemaining
	 *            minutes remaining on user account
	...
	public String addSong(Song song) {
		...
		if (valid == 0) {
			...
			if (valid == 1)
				return "Can not add more songs. Maximum song limit has been reached.";

			else if (valid == 2)
				return "Song has already been played maximum amount of times.";

			else
				return timeRemaining + " minutes left in account.\nSong is too long to add to queue";
		}
	}

	/**
	 * Validates songs according to the following criteria: - A song can be played a
	 * maximum of 3 times per day. - A maximum number 3 songs per day may be played.
	...
	private int validateSong(Song song) {
		...
		if (NumSongsSelected >= MAX)
			return 1;

		if (song.getNumTimesPlayed() >= MAX)
			return 2;

		if (timeRemaining < song.getSongLength())
			return 3;
